...
export default function App() {
  const types = {
    ...
    income: {
      ...
      recommendation:
        '役員報酬・利益・内部留保の配分設計を先に見ると、かなり効きます。'
    },
    growth: {
      id: 'growth',
      name: '成長・拡大型',
    ...
    succession: {
      ...
      recommendation:
        '後継者育成より先に、社長依存の棚卸しを始めると整理しやすくなります。'
    },
    exit: {
      id: 'exit',
      name: '売却・資本政策型',
      ...
      recommendation:
        '“社長を買う案件”ではなく“仕組みを買う案件”に近づける意識が重要です。'
    }
  }
          ...
          <section className="grid gap-6 lg:grid-cols-[1.28fr_0.72fr]">
            ...
            <aside className="space-y-6">
              <div className="rounded-[28px] border border-stone-200 bg-white p-6 shadow-[0_10px_30px_rgba(28,25,23,0.04)]">
                ...
                <p className="mt-3 text-sm leading-7 text-stone-600">
                  {totalAnswered} / {questions.length} 問に回答済みです。
                </p>
                <button
                  disabled={!complete}
            ...
            <div className="rounded-[28px] border border-stone-200 bg-white p-7 shadow-[0_10px_30px_rgba(28,25,23,0.04)] sm:p-8">
              ...
              {result.isHybrid && (
                <p className="mt-4 text-sm leading-7 text-stone-600">
                  ...
                  二つの重心を持つ経営としてKGI/KPIを設計した方が、現実に合いやすい状態です。
                </p>
              )}
              ...
              <div className="mt-8 rounded-[28px] border border-stone-200 bg-stone-50 p-6 sm:p-7">
                ...
                <h3 className="mt-3 text-2xl font-semibold tracking-tight">
                  結果を見て終わりにせず、実際の数字まで整理したい方へ。
                </h3>
                <p className="mt-4 text-sm leading-7 text-stone-700">
                  この診断は、あくまで入口です。実際には、同じ「{result.top.name}」でも、
                  業種・成長段階・借入状況・オーナーの意向によって、追うべきKGI/KPIは少しずつ変わります。
                  九十九アドバイザリーでは、診断結果をたたき台にしながら、貴社の数字と現場に合わせて、
                  無理のない優先順位を整理します。
                </p>

                <div className="mt-5 grid gap-4 sm:grid-cols-3">
                  <div className="rounded-2xl border border-stone-200 bg-white p-4">
                    <p className="text-sm font-semibold text-stone-900">初回で整理すること</p>
                    <p className="mt-2 text-sm leading-6 text-stone-600">
                      いま何をKGIに置くべきか、見るべき数字は何か、どこから整えるべきかを静かに言語化します。
                    </p>
                  </div>
                  <div className="rounded-2xl border border-stone-200 bg-white p-4">
                  ...
                  <div className="rounded-2xl border border-stone-200 bg-white p-4">
                    <p className="text-sm font-semibold text-stone-900">相談後に持ち帰れること</p>
                    <p className="mt-2 text-sm leading-6 text-stone-600">
                      貴社に合ったKGIの仮置き、月次で追うKPI候補、次に整えるべき論点の優先順位です。
                    </p>
                  </div>
                </div>
